...
# Source: NSE / SEBI classification as of 2024
# Nano cap  : MCap < ₹500 Cr   (ultra-neglected, high discovery potential)
# Small cap : ₹500 Cr – ₹5,000 Cr  (SEBI small-cap index threshold ~₹5,000 Cr)
# Mid cap   : ₹5,000 Cr – ₹50,000 Cr (retained as "Extended Small" option)
#
# Radio option "Nano only"        → 0 – 500 Cr
# Radio option "Small cap only"   → 500 – 5,000 Cr
# Radio option "Nano + Small"     → 0 – 5,000 Cr
# Radio option "Extended (up to Mid)" → 0 – 20,000 Cr  (wider net, user's choice)
...
# NOTE: for small/nano caps the quality bar is intentionally kept
# identical or slightly tighter, because the mandate is
# "world-class quality available cheap", NOT "any small cap".
CONFIG: Dict[str, Any] = {
    "pe_max":           25.0,   # slightly wider than v6 (20) to catch nano mispricing
    "peg_max":          1.2,
    "ev_ebitda_max":    15.0,
    "pb_max":           4.0,    # small caps with high ROE can have >3 PB legitimately
    "roce_min":         0.18,
    "roe_min":          0.15,
    "roa_min":          0.08,
    "opm_min":          0.12,
    ...
    "de_max_guard":     1.00,
    "promoter_min":     0.35,   # slightly relaxed for nano caps
    "insider_min":      0.35,
    "quality_min_guard":3,      # relaxed for nano (fewer Yahoo fields available)
}
...
def normalise_company_name(s: str) -> str:
    if s is None:
        return ""
    s = str(s).upper().strip()
    for token in ["LIMITED", "LTD", "LIMITED.", "LTD.", "INDIA", "(INDIA)", "INDIAN",
                  "PRIVATE", "PVT", "PVT.", "&", ",", ".", "-", "/", "(", ")"]:
        s = s.replace(token, " ")
    return " ".join(s.split())
...
# ═══════════════════════════════════════════════════════════
# UNIVERSE BUILDER  ← KEY CHANGE IN V7
# ═══════════════════════════════════════════════════════════
...
def build_nse_equity_universe(nse_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Return ALL equities sorted by turnover — MCap filter applied later
...
def apply_mcap_filter(
    ...
    within [mcap_min_cr, mcap_max_cr].  Returns filtered df + total_fetched count.
    
    To keep startup time reasonable, we walk tickers in turnover order
    and stop once we have collected `max_stocks` valid ones.
    """
    kept: List[Dict] = []
    checked = 0
...
def compute_screen_verdict(
    ...
    genuine_fail = any(
        (not p) and (not m)
        for p, m in zip([l1_val, l2_prof, l3_guard, l4_share, l5_guard],
                        [l1_dm,  l2_dm,  l3_dm,  l4_dm,  l5_dm])
    )
    if genuine_fail:
...
def evaluate_stock(ticker: str, weights: Dict[str, int]) -> Dict[str, Any]:
    try:
        ...
        return {
            ...
            "L5_DataMissing": True,
            "Conviction": 0, "WeightedScore": 0.0, "Pass": False,
...
# ★ SIZE BAND RADIO — the key new control
size_choice = st.sidebar.radio(
    "📐 Cap size universe",
    options=list(MCAP_BANDS.keys()),
    index=2,   # default: Nano + Small
    help=(
...
max_stocks   = st.sidebar.number_input(
    "Max stocks to collect in size band", min_value=10, max_value=1000, value=200, step=25,
...
feeder_pool_size = st.sidebar.number_input(
    "Bhavcopy pool to walk (for MCap filter)", min_value=100, max_value=5000, value=2000, step=100,
...
if st.button("🚀 Run live screen", type="primary"):
    ...
    if equity_universe_df_local is not None and not equity_universe_df_local.empty:
        ...
        # ── Two-stage approach: use MCap from fundamentals_master if available,
        #    else use Yahoo live fetch (slower but works without master file)
        if (fundamentals_master_df is not None
                and not fundamentals_master_df.empty
                and "MCap_Cr" in fundamentals_master_df.columns):
            # Fast path — merge MCap from master
            ...
            # Slow path — live Yahoo MCap fetch
            st.warning(
    ...
    if not df.empty and stock_master_df is not None and not stock_master_df.empty:
        ...
        if len(merge_cols) > 1:
            df = df.merge(stock_master_df[merge_cols], on="Ticker",
    ...
    # Enrich with fundamentals_master
    if not df.empty and fundamentals_master_df is not None and not fundamentals_master_df.empty:
        ...
        df = df.merge(fundamentals_master_df[fund_merge_cols], on="Ticker",
    ...
    # MCap band breakdown of screened universe
    band_breakdown = (
        summary_df_before_pass_filter.groupby("MCap_Band")["Ticker"]
        .count().reset_index().rename(columns={"Ticker": "Count"})
    )
    ...
    fail_detail = summary_df_before_pass_filter[
        ...
        "Ticker", "MCap_Cr", "MCap_Band", "Conviction", "WeightedScore",
    ...
    if not df.empty:
        ...
        st.download_button(
            "⬇️ Download full screen CSV (all 500)",
            data=summary_df_before_pass_filter.to_csv(index=False),
            file_name="100x_screener_v7_full.csv",
            mime="text/csv",
        )
    else:
        st.info("No stocks passed the current filters. Try relaxing conviction score or widening the MCap band.")

else:
    st.info("👆 Upload files in the sidebar, choose your cap size band, then click **Run live screen**.")

    # Static reference table shown on landing
