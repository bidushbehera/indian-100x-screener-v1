...
from config import (
    BASE_DIR, CACHE_DIR, RUNS_DIR, MANUAL_COMPANY_DIR, MANUAL_SECTOR_DIR,
    CONFIG, DEFAULT_UNIVERSE, SCHEMA_VERSION, CACHE_MAX_AGE_DAYS,
    QUARTERLY_REPORT_SOURCES, SECTOR_REPORT_SOURCES,
)
...
def build_shareholding_lookup(shareholding_df: pd.DataFrame, stock_master_df: Optional[pd.DataFrame] = None) -> Dict[str, Dict]:
    ...
    for _, row in df.iterrows():
        key = normalise_company_name(str(row[SH_COL_COMPANY]).strip())
    ...
    if stock_master_df is not None and not stock_master_df.empty:
        name_col = next((c for c in ["Company", "CompanyName", "Company Name", "Name"] if c in stock_master_df.columns), None)
        if name_col and "Ticker" in stock_master_df.columns:
            for _, row in stock_master_df.iterrows():
                stock_name_to_ticker[normalise_company_name(row[name_col])] = str(row["Ticker"]).upper()
    for nkey, row in company_to_row.items():
        ...
        lookup[ticker] = {
            ...
            "OwnershipDataValid": promoter_pct is not None and public_pct is not None and total is not None and abs(total - 100.0) < 5.0,
            "ShareholdingAsOnDate": str(row[SH_COL_AS_ON]).strip() if SH_COL_AS_ON in row.index else None,
            "ShareholdingRevisionDate": str(row[SH_COL_REVISION]).strip() if SH_COL_REVISION in row.index else None,
            "ShareholdingActionLink": str(row[SH_COL_ACTION]).strip() if SH_COL_ACTION in row.index else None,
...
def compute_screen_verdict(l1_val, l2_prof, l3_guard, l4_share, l5_guard, l1_data_missing, l2_data_missing, l3_data_missing, l4_data_missing, l5_data_missing):
    ...
    if any((not p) and (not m) for p, m in zip(layers_pass, layers_missing)):
        return VERDICT_FAIL_GENUINE
    if any(layers_missing):
        return VERDICT_PASS_DATAGAP
    return VERDICT_PASS

def evaluate_stock(ticker: str, weights: Dict[str, int], fundamentals_lookup: Dict[str, Any], shareholding_lookup: Dict[str, Dict]) -> Dict[str, Any]:
    try:
        ...
        if ebit and ta and current_liab is not None:
            cap_emp = ta - current_liab
            if cap_emp > 0:
                roce = ebit / cap_emp
        if fund_row is not None:
            for fm_col, var_name in [("ROE_Latest", "roe"), ("ROCE_Latest", "roce"), ("OPM_Latest", "opm"), ("Revenue_CAGR_AllYears", "revg"), ("PAT_CAGR_AllYears", "earng")]:
        ...
        peg = pe / (earng * 100.0) if pe and earng and earng > 0 else None
        ocf_pat = ocf / ni if ocf and ni and ni > 0 else None
        fcf_yield = fcf / mcap_raw if fcf and mcap_raw else None
        de_ratio = float(de) if de is not None and de >= 0 and float(de) <= 10 else None
        quality_raw = approx_quality_score(info)
        promoter_pct_nse = sh_data.get("PromoterPct_NSE") if sh_data else None
        if promoter_pct_nse is not None:
            l4_share = sh_data.get("OwnershipDataValid", False) and (promoter_pct_nse / 100.0) >= CONFIG["promoter_min"]
            l4_data_missing = False
            ownership_score = 1.0 if l4_share else 0.0
            share_status = "NSE CSV"
        else:
            ...
            share_status = "yfinance (fallback)" if insider is not None else "Not available"
        l1_checks = [pe is not None and pe < CONFIG["pe_max"], peg is not None and peg < CONFIG["peg_max"], ev_ebitda is not None and ev_ebitda < CONFIG["ev_ebitda_max"], pb is not None and pb < CONFIG["pb_max"], mcap_cr is not None]
        ...
        l2_checks = [roce is not None and roce > CONFIG["roce_min"], roe is not None and roe > CONFIG["roe_min"], roa is not None and roa > CONFIG["roa_min"], opm is not None and opm > CONFIG["opm_min"], revg is not None and revg > CONFIG["rev_growth_min"], earng is not None and earng > CONFIG["earn_growth_min"]]
        ...
        l5_fields_present = sum([safe(info, "netIncomeToCommon") is not None, safe(info, "operatingCashflow") is not None, safe(info, "returnOnAssets") is not None, safe(info, "longTermDebt") is not None, safe(info, "totalAssets") is not None, safe(info, "currentRatio") is not None, safe(info, "grossMargins") is not None])
        l5_data_missing = l5_fields_present < 4
        l3_guard = (ocf_pat is not None and ocf_pat >= CONFIG["ocf_pat_min_guard"]) and (de_ratio is None or de_ratio <= CONFIG["de_max_guard"])
        l5_guard = quality_raw >= CONFIG["quality_min_guard"]
        verdict = compute_screen_verdict(l1_val, l2_prof, l3_guard, l4_share, l5_guard, l1_data_missing, l2_data_missing, l3_data_missing, l4_data_missing, l5_data_missing)
        l3_score = 0.0
        if ocf_pat is not None: l3_score += 1.0 if ocf_pat >= 1.2 else 0.8 if ocf_pat >= 0.8 else 0.5 if ocf_pat >= 0.5 else 0.0
        if fcf_yield is not None: l3_score += 1.0 if fcf_yield >= 0.05 else 0.7 if fcf_yield >= 0.03 else 0.4 if fcf_yield > 0 else 0.0
        if de_ratio is not None: l3_score += 1.0 if de_ratio <= 0.3 else 0.8 if de_ratio <= 0.5 else 0.4 if de_ratio <= 1.0 else 0.0
        ...
        weighted_score += weights["l3_ocf_pat"] * (1.0 if ocf_pat is not None and ocf_pat >= 0.8 else 0.5 if ocf_pat is not None and ocf_pat >= 0.5 else 0.0)
        weighted_score += weights["l3_fcf_yield"] * (1.0 if fcf_yield is not None and fcf_yield >= 0.03 else 0.5 if fcf_yield is not None and fcf_yield > 0 else 0.0)
        weighted_score += weights["l3_debt"] * (1.0 if de_ratio is not None and de_ratio <= 0.5 else 0.5 if de_ratio is not None and de_ratio <= 1.0 else 0.0 if de_ratio is not None else 0.25)
        weighted_score += weights["l5_forensic"] * l5_score
        weighted_score += weights["ownership"] * ownership_score
        return {
            "Ticker": base_ticker, "Price": price, "MCap_Cr": round(mcap_cr, 1) if mcap_cr is not None else None,
            "PE": round(pe, 2) if pe is not None else None, "PB": round(pb, 2) if pb is not None else None,
            "PEG": round(peg, 2) if peg is not None else None, "ROCE_pct": round(roce * 100, 1) if roce is not None else None,
            "ROE_pct": round(roe * 100, 1) if roe is not None else None, "ROA_pct": round(roa * 100, 1) if roa is not None else None,
            "OPM_pct": round(opm * 100, 1) if opm is not None else None, "RevGrowth_pct": round(revg * 100, 1) if revg is not None else None,
            "EarnGrowth_pct": round(earng * 100, 1) if earng is not None else None, "OCF_PAT": round(ocf_pat, 2) if ocf_pat is not None else None,
            "FCFYield_pct": round(fcf_yield * 100, 2) if fcf_yield is not None else None, "DebtToEquity_raw": round(de_ratio, 2) if de_ratio is not None else None,
            "ScreenVerdict": verdict, "Conviction": conviction, "WeightedScore": round(weighted_score, 2),
            "ShareholdingStatus": share_status, "PromoterPct_NSE": promoter_pct_nse, "Pass": bool(l2_prof and l3_guard and l5_guard and conviction >= 4),
            "L1_DataMissing": l1_data_missing, "L2_DataMissing": l2_data_missing, "L3_DataMissing": l3_data_missing, "L4_DataMissing": l4_data_missing, "L5_DataMissing": l5_data_missing,
            "Error": None,
        }
    except Exception as e:
        return {"Ticker": ticker.replace(".NS", ""), "ScreenVerdict": VERDICT_FAIL_NODATA, "Conviction": 0, "WeightedScore": 0.0, "Pass": False, "Error": str(e)}
...
def run_pipeline() -> Tuple[Path, pd.DataFrame, Dict[str, Any]]:
    ...
    if bh.path and bh.path.suffix.lower() == ".csv":
        bhav_df = load_csv_if_exists(bh.path)
    ...
    stock_master_df = load_csv_if_exists(BASE_DIR / "stock_master.csv")
    fundamentals_master_df = load_csv_if_exists(BASE_DIR / "fundamentals_master.csv")
    fundamentals_lookup = {}
    if not fundamentals_master_df.empty and "Ticker" in fundamentals_master_df.columns:
    ...
    weights = {"l3_ocf_pat": 4, "l3_fcf_yield": 3, "l3_debt": 4, "l5_forensic": 4, "ownership": 3}
    rows = [evaluate_stock(t, weights, fundamentals_lookup, shareholding_lookup) for t in tickers]
    ...
    quarterly_available = {t: any((MANUAL_COMPANY_DIR / t).glob("*")) for t in pass_df["Ticker"].tolist()} if not pass_df.empty else {}
    sector_available = {t: any(MANUAL_SECTOR_DIR.glob("*")) for t in pass_df["Ticker"].tolist()} if not pass_df.empty else {}
...
def render_app():
    ...
    st.caption("Automated screening + manual intervention handling + alerts + research notes")
    ...
    if not pass_names:
        ...
        for ticker in pass_names:
            ...
            with st.expander(f"{ticker}", expanded=True):
                ...
                st.markdown("**Quarterly report upload**")
                st.caption("Upload latest official quarterly results / presentation / filing if automation could not retrieve them reliably.")
                st.file_uploader(f"Upload quarterly files for {ticker}", accept_multiple_files=True, key=f"company_{ticker}")
                ...
                st.markdown("**Sector / industry report upload**")
                st.caption("Upload latest free public sector or industry report if auto-ingestion failed.")
                st.file_uploader(f"Upload sector reports for {ticker}", accept_multiple_files=True, key=f"sector_{ticker}")
